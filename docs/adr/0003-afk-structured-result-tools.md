# Use token-gated AFK result tools for role completion

AFK role sessions report implementer, quality, and verifier outcomes by calling AFK-owned terminating result tools instead of ending with manually formatted JSON text. The AFK extension issues a per-role token, gates `afk_role_result` and `afk_verify_result` calls against an active token registry, and consumes an atomic `.pi/afk/results/<token>.json` handoff because this preserves structured completion semantics without relying on prompt-only JSON compliance.
...
- Modify a role-runner backend to add structured output: rejected because AFK can use Pi extension tools directly while avoiding backend-specific coupling.
