...
Unengaged staging layouts distribute Defenders equally per Defend cell. Layouts first minimize incumbent relocation, then current-cell relocation. Small cohorts minimize total travel exactly after those continuity constraints; large cohorts choose the remaining destinations through deterministic bounded-nearest matching. This bounded cut keeps paint edits inside the next-step frame budget rather than treating globally perfect travel as more authoritative than responsiveness. Cross-cell redistribution uses normal movement speed; local roaming is gentle and continuous. Without Defend paint, Defenders stage across Swarm Tiles, and without any Swarm Tiles they roam procedurally inside their current cells. [ADR-0018](0018-independent-swarm-intent.md) replaces Defend Contests with independent overlapping paint; physical presence never captures or erases intent.
...
The swarm-wide rotation cap governs admission. An accepted rotation covers travel to the Charger and charging there: if casualties reduce the cap below the active count, those rotations may finish. New rotations wait until the active count falls below the current cap; casualties alone do not cancel an accepted trip or interrupt charging. If the assigned Charger is destroyed or becomes invalid, the rotation ends; selecting another Charger requires fresh admission under the current cap. This preserves accepted service through casualties without reserving capacity after that service becomes unavailable.
