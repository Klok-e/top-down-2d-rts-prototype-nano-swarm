...
#[test]
#[ignore = "requires a GPU adapter; run with cargo test --test playtest experiment_runtime_saves_unresolved_cutoff -- --ignored"]
fn experiment_runtime_saves_unresolved_cutoff_before_requesting_exit() {
    let directory = RunDirectory(std::env::temp_dir().join(format!(
        "nano-battle-experiment-runtime-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
    ...
    let run = fs::read_dir(&directory.0)
        .unwrap()
        .next()
        .unwrap()
        .unwrap()
    ...
    assert_eq!(summary["latest"]["fixed_tick"], 60);
    assert_eq!(summary["experiment"]["controllers"][0], "timed");
    assert_eq!(summary["experiment"]["controllers"][1], "timed");
    assert_eq!(summary["experiment"]["layout"], "narrows");
    assert_eq!(summary["experiment"]["swap_sides"], true);
    assert_eq!(summary["experiment"]["cutoff_seconds"], 1);
    assert_eq!(summary["latest"]["controllers"]["0"]["reviews"], 0);
    assert_eq!(summary["latest"]["controllers"]["1"]["reviews"], 0);
    assert!(summary["latest"]["observation_max_ms"].as_f64().unwrap() > 0.0);
}
